...
moreSlackInteractions.viewSubmission(
    ...
    async (payload, respond) => {
        ...
        const removePlan = await axios.delete(
            `${urlString}/programs/selectedProgram/delete-program/${user}`
        )
...
moreSlackInteractions.viewSubmission(
    'selected_program_workouts_index',
    async (payload, respond) => {
        const findToken = await OAuth.findOne({ team_id: payload.team.id })
        const webAPI = web(findToken.access_token)
        const metadata = JSON.parse(payload.view.private_metadata)
...
moreSlackInteractions.viewSubmission(
    ...
    async (payload, respond) => {
        try {
            ...
            const metadata = JSON.parse(payload.view.private_metadata)
            const { home_or_slash } = metadata
//Start public_private
            const { public_private } = payload.view.state.values.type

            if (
                public_private.selected_option !== null &&
...
moreSlackInteractions.viewSubmission(
    ...
    async (payload, respond) => {
        try {
            ...
            const sendGoals = await axios.post(
                `${urlString}/weeklyGoals/${user_id}`,
                data
            )
...
moreSlackInteractions.viewSubmission(
    ...
    async (payload, respond) => {
        try {
            ...
            const sendGoals = await axios.put(
                `${urlString}/weeklyGoals/${id}`,
                data
            )
...
moreSlackInteractions.viewSubmission(
    'view_calendar_workouts',
    async (payload, respond) => {
        console.log(
            'Not sure what to do with view_calendar_workouts in more-slack-interactions'
        )
    }
)
                ...
                attachments: [
                    {
                        ...
                        text:
                            'Thank you for your input! Your feedback has been submitted and will be reviewed.',
                    },
                ],
            },
            config
