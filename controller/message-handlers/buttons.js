  ...
  try {
    ...
    if (value === "calendar") {
      ...
      if (payload.view.callback_id === "homepage_modal") {
        const calendar = await viewCalendarWorkouts(
          payload,
          allWorkouts,
          "slash"
        );
    ...
    if (home_or_slash === "slash") {
      const updated = await updatedCalendarWorkouts(
        payload,
        payload.view.id,
        workouts,
        "slash"
  ...
  try {
    ...
    if (value === "program_workouts") {
      const workouts = await axios.get(
        `${urlString}/programs/selectedProgram/get-workouts/${user_id}`
      );
      if (payload.view.callback_id === "homepage_modal") {
        const listWorkouts = await selectedProgramWorkouts(
          payload,
          workouts,
          "slash"
        );
    ...
    //pagination for selected program workouts
      ...
      if (home_or_slash === "slash") {
        ...
        const selectedProgramIndex = await updatedProgramWorkouts(
          payload,
          payload.view.id,
          workouts,
          "slash"
    ...
    // ENTER SCORE on today's scheduled workout
      ...
      if (home_or_slash === "slash") {
        if (payload.view.callback_id === "homepage_modal") {
          const submitTimeView = await submitTime(
            payload,
            workoutSelected[0],
            "slash",
            "yes"
    ...
    // ENTER SCORE cf workout of the day
      ...
      console.log({ payload });
      if (
        payload.view?.callback_id === "homepage_modal" ||
        payload.view?.callback_id === "help"
      ) {
    ...
    //COMPLETE WORKOUT viewWorkouts inside of createWorkouts
      ...
      if (workoutSelected[0].type === "Reps") {
        ...
        if (home_or_slash === "slash") {
          const modal = await roundsPlusRepsModal(
            payload,
            workoutSelected[0],
            "slash"
          );
        ...
        if (home_or_slash === "slash") {
          const modal = await distanceModal(
            payload,
            workoutSelected[0],
            "slash"
          );
    ...
    // EDIT WORKOUT inside of Workouts Created editWorkout
      ...
      if (home_or_slash === "slash") {
        const awaitWorkouts = await editWorkout(
          payload,
          workoutSelected[0],
          "slash"
        );
    ...
    // DELETE WORKOUT inside of Workouts Created
      ...
      buttonPressed = buttonPressed.replace("delete", "");
      const deleteWorkout = await axios.delete(
        `${urlString}/slack/delete-workout/${buttonPressed}`
      );
      ...
      if (home_or_slash === "slash") {
        const updatedIndex = await updatedWorkouts(
          payload,
          payload.view.id,
          workouts,
          "slash"
    ...
    //REDO WORKOUT inside viewCompleteWorkouts
      ...
      viewId = payload.container.view_id;
      buttonPressed = buttonPressed.replace("complete", "");
      const workoutSelected = await FinishedWorkout.find({
        _id: buttonPressed,
      });
      //
      //
    ...
    //EDIT COMPLETE WORKOUT inside of Workouts Completed createWorkouts/editWorkout.js
      ...
      if (home_or_slash === "slash") {
        const edit = await editCompletedWorkout(
          payload,
          workoutSelected[0],
          "slash"
        );
    ...
    //DELETE WORKOUUT inside of Workouts Completed
      ...
      const metadata = JSON.parse(payload.view.private_metadata);
      const { home_or_slash } = metadata;
      const deleteWorkout = await FinishedWorkout.deleteOne({
        _id: buttonPressed,
      });
      const workouts = await axios.get(
      ...
      if (home_or_slash === "slash") {
        const workoutIndex = await updatedCompletedWorkouts(
          payload,
          payload.view.id,
          workouts,
          "slash"
    ...
    //Pgination for complete workouts
      ...
      if (home_or_slash === "slash") {
        const updatedIndex = await updatedCompletedWorkouts(
          payload,
          payload.view.id,
          workouts,
          "slash"
    ...
    //REDO WORKOUT inside viewCalendar
      ...
      if (workoutSelected[0].type === "Reps") {
        if (home_or_slash === "slash") {
          const modal = await calendarReps(
            payload,
            workoutSelected[0],
            "slash"
          );
        ...
        if (home_or_slash === "slash") {
          console.log("whatever");
          const modal = await calendarRoundsPlusReps(
            payload,
            workoutSelected[0],
            "slash"
          );
        ...
        if (home_or_slash === "slash") {
          const modal = await calendarTime(
            payload,
            workoutSelected[0],
            "slash"
          );
        ...
        if (home_or_slash === "slash") {
          const modal = await calendarLoad(
            payload,
            workoutSelected[0],
            "slash"
          );
        ...
        if (home_or_slash === "slash") {
          const modal = await calendarOther(
            payload,
            workoutSelected[0],
            "slash"
          );
        ...
        if (home_or_slash === "slash") {
          const modal = await calendarDistance(
            payload,
            workoutSelected[0],
            "slash"
          );
        ...
        if (home_or_slash === "slash") {
          const modal = await calendarMeters(
            payload,
            workoutSelected[0],
            "slash"
          );
    ...
    //EDIT COMPLETE WORKOUT inside of CALENDAR Workouts calendar/editCalendarWorkout.js
      ...
      if (home_or_slash === "slash") {
        const edit = await editCalendarWorkout(
          payload,
          workoutSelected[0],
          "slash"
        );
    ...
    //DELETE WORKOUT inside of Workouts Completed
      ...
      if (home_or_slash === "slash") {
        const workoutIndex = await updatedCalendarWorkouts(
          payload,
          payload.view.id,
          workouts,
          "slash"
    ...
    //AUTHORIZE STRAVA (at the very top of the page)
      ...
      const createUser = await User.findOneAndUpdate(
        { user_id: id },
        { $set: { team_id: team_id, user_name: name } },
