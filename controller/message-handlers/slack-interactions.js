...
slackInteractions.viewSubmission("create_workout", async (payload, respond) => {
  try {
    ...
    const sendWorkout = await axios.post(
      `${urlString}/slack/create-workout/${user_id}`,
      data
    );
...
slackInteractions.viewSubmission(
  ...
  async (payload, respond) => {
    try {
      ...
      type =
        type.edit.selected_option == null
          ? payload.view.blocks[0].accessory.placeholder.text
          : type.edit.selected_option.value;
      ...
      const sendWorkout = await axios.put(
        `${urlString}/slack/edit-workout/${id}`,
        data
      );
      ...
      if (home_or_slash === "slash") {
        const updated = await updatedWorkouts(
          payload,
          payload.view.previous_view_id,
          workouts,
          "slash"
        );
...
slackInteractions.viewSubmission(
  ...
  async (payload, respond) => {
    try {
      ...
      const user_id = payload.user.id;
      var data;
      var { minutes, seconds, rounds, reps, weight, miles, meters, notes } =
      ...
      console.log("\n\n I don't get it");
      const radioButton =
        payload.view.state.values.radio["radio_buttons-action"].selected_option
          .value;
      const user = payload.user.id;
      const userInfo = await webAPI.users.info({ user: user });
      const passUser = userInfo.user;
      ...
      if (home_or_slash === "slash") {
        if (action === "complete_completed_workouts") {
          ...
          const updated = await updatedCompletedWorkouts(
            payload,
            payload.view.previous_view_id,
            workouts,
            "slash"
...
slackInteractions.viewSubmission(
  ...
  async (payload, respond) => {
    try {
      ...
      const sendWorkout = await axios.put(
        `${urlString}/finishedWorkouts/edit/${id}`,
        data
      );
      const workouts = await axios.get(
        `${urlString}/finishedWorkouts/${user_id}`
  ...
  async (payload, respond) => {
    try {
      ...
      const subscribe = await axios.post(
        `${urlString}/programs/selectedProgram/subscribe/${user_id}/${distance}`,
        { startDate: date }
      );
      const user = payload.user.id;
      const userInfo = await webAPI.users.info({ user: user });
...
slackInteractions.viewSubmission(
  ...
  async (payload, respond) => {
    try {
      ...
      const sendWorkout = await axios.post(
        `${urlString}/programs/selectedProgram/enter-score/${user_id}/${id}`,
        data
      );
      const user = payload.user.id;
      const userInfo = await webAPI.users.info({ user: user });
      ...
      if (home_or_slash === "slash") {
        ...
        const updated = await updatedProgramWorkouts(
          payload,
          payload.view.previous_view_id,
          workouts,
          "slash"
      ...
      // webAPI.views.update(updated)
      webAPI.views.publish(
        homepage(passUser, allWorkouts, wod[0], publicChannels)
      );
      return;
    } catch (err) {
      console.error(err.message);
